...
export default function NavbarChatList(props:any){
    const{directChatProfiles, groupChats, channels} = useContext(WorkspaceContext)
    return(
        <div className='navbar_chatList'>
            ...
            <NavbarChatListMenu title={'Channels'} key={1}>
                ...
                channels.map((channel:any)=>(   
                        <ChannelMenu key={channel.id} title={channel.name} chatID={channel.id} svgComponent={ChannelIconElement()}/>
                    ))
                }
            </NavbarChatListMenu>
            ...
            <NavbarChatListMenu title={'Direct Messages'} key={2}>
            {
                    directChatProfiles.map((profile:any)=>(   
                        <DirectMessageUser key={profile.id} profileID={profile.id} profileName={profile.name} profileAvatar={profile.imageSrc}/>
                    ))
                } 
            </NavbarChatListMenu >
            ...
            <NavbarChatListMenu title={'Group Messages'} key={3}>
            {
                   groupChats.map((chat:any)=>(   
                        <GroupChatMenu key={chat.id} groupName={chat.name} groupID={chat.id} groupAvatar={chat.imageSrc}/>
                    ))
                }  
            </NavbarChatListMenu>
        </div>
