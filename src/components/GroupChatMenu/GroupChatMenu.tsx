...
export default function GroupChatMenu(props:{groupID:number, groupName:string, groupAvatar:string }){
    ...
    return(
        <button className={'button navbar_quick_access_menu'}
            id = {selectedChatWindow.type ===CHAT_TYPE.GROUP && selectedChatWindow.id === props.groupID? 'active':''} 
            onClick={()=>setSelectedChatWindow({type:CHAT_TYPE.GROUP,id:props.groupID})}>
