...
export default function WorkspaceHeader(){
    const {selectedChatWindow, directChatProfiles, groupChats, channels} = useContext(WorkspaceContext);
    const [currentUser, setCurrentUser] = useState<ChatUser>({} as ChatUser);

    const getChatInfoFromID = ()=>{
        const searchList = selectedChatWindow.type === CHAT_TYPE.DIRECT? directChatProfiles: selectedChatWindow.type === CHAT_TYPE.CHANNEL? channels: groupChats
        console.log(searchList)
        const chatInfo = searchList.filter((itm:ChatUser)=> itm.id === selectedChatWindow.id)
