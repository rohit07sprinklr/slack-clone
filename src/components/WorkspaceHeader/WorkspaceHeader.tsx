...
export default function WorkspaceHeader(){
    const {selectedChatId, directChatProfiles} = useContext(WorkspaceContext);
    const [currentUser, setCurrentUser] = useState<ChatUser>({} as ChatUser);

    const getUserFromID = ()=>{
        const user = directChatProfiles.filter((itm:ChatUser)=> itm.id === selectedChatId)
    ...
    useEffect(()=>{
        ...
        setCurrentUser(user);
    },[selectedChatId,directChatProfiles])
